...
class LightifyRGBWZigBee extends ZigBeeDevice {
	onMeshInit() {
		...
		this.registerCapability('light_temperature', 'lightingColorCtrl', {
			set: 'moveToColorTemp',
			setParser: value => {
				this.log('light_temp setParser', value);
				return {
		...
		this.registerCapability('light_mode', 'lightingColorCtrl', {
			...
			setParser: value => {
				...
				switch (value) {
					case "temperature":
						this.log('do temperature', this.getCapabilityValue('light_temperature'))
						...
						break;
					case "color":
						this.log('do color', this.node.endpoints[0].clusters['lightingColorCtrl']);
						this.node.endpoints[0].clusters['lightingColorCtrl']
							...
							.catch(err => {
								this.error('color error', err)
								return new Error('failed_to_do_move_to_hue_and_saturation')
							});
						break;
					default:
						return null;
